...
from modules.mod_exploration import (
    generer_rapport_pdf,
    telecharger_donnees,
    traiter_outliers,
    detection_outliers,
...
with onglet1:
    ...
    st.dataframe(
        df.dtypes.reset_index().rename(columns={0: "Type", "index": "Colonne"})
    )
    ...
    # Autodétection
    auto_detected_task = (
        "classification" if df[target].nunique() <= 10 else "régression"
    )
    ...
    task = st.radio(
        ...
        index=0 if auto_detected_task == "classification" else 1,
    )
    ...
    st.write("***")
    st.markdown(
        "### Vous pouvez maintenant passer à l'onglet suivant : 📊 Distributions"
    )
...
with onglet2:
    ...
    st.write("***")
    st.markdown(
        "### Vous pouvez maintenant passer à l'onglet suivant : 📈 Corrélations"
    )
...
with onglet3:
    ...
    if task == "classification" and not pd.api.types.is_numeric_dtype(df_corr[target]):
        ...
        if choix_encoder == "Label Encoding":
            ...
            drop_first = st.checkbox(
                "Supprimer la première modalité (évite la multicolinéarité)",
                value=False,
            )
        ...
        if st.button("Appliquer l'encodage"):
            df_corr = df.copy()
            df_corr, encoded_target_name = encoder_cible(
                df_corr, target, choix_encoder, drop_first
            )
        ...
        if "choix_encoder" in st.session_state:
            st.info(
                f"Encodage actuellement appliqué : `{st.session_state['choix_encoder']}`"
            )
    else:
        st.session_state["df_corr"] = df_corr
        ...
        if col != target_corr:
            ...
            if abs(val_corr) < seuil_bas:
                to_drop.append(
                    {
                        "Colonne": col,
                        "Corrélation avec la cible": corr.at[col, target_corr],
                    }
                )
    ...
    if corr_trop_fortes:
        df_corr_fortes = pd.DataFrame(corr_trop_fortes).sort_values(
    ...
    if isinstance(target_corr, list):
        ...
        if not cibles_valides:
            st.warning(
                "Aucune colonne cible encodée n'est présente dans la matrice de corrélation."
            )
            good_corr_cols = []
        else:
            ...
            good_corr_cols = (
                corr_scores[filtres_target]
                .drop(labels=cibles_valides, errors="ignore")
                .index.tolist()
            )
    else:
        if target_corr not in corr.columns:
            st.warning(
                f"La colonne cible '{target_corr}' n'est plus présente dans les données."
            )
            good_corr_cols = []
            ...
            filtres_target = (abs(corr[target_corr]) > seuil_bas) & (
                abs(corr[target_corr]) <= 0.95
            )
    ...
    selected_cols_corr = st.multiselect(
        "Colonnes à conserver pour la modélisation",
        numeric_cols,
        default=good_corr_cols,
    )
    ...
    if st.button("✅ Valider cette version comme jeu de données nettoyé"):
        ...
        st.success(
            "La version avec encodage a été validée comme jeu de données nettoyé (df_clean)."
        )

        st.write("***")
        st.markdown(
            "### Vous pouvez maintenant passer à l'onglet suivant : 🧹 NaN & Outliers"
        )
...
with onglet4:
    if "df_clean" in st.session_state:
        ...
        selected_nan = st.multiselect(
            "Colonnes à traiter", nan_summary[nan_summary > 0].index.tolist()
        )
        ...
        numeric_cols = df.select_dtypes(
            include=np.number
        ).columns.tolist()  # recalcul après potentiel suppression de colonnes
        outliers_bool = detection_outliers(
            df, numeric_cols, methode_outlier, key="zscore1"
        )

        st.write("Nombre d'outliers par colonne :")
        st.write(outliers_bool.sum())

        traitement_outlier = st.selectbox(
            "Action", ["Aucune", "Supprimer les lignes", "Remplacer par la médiane"]
        )

        if st.button("Remplacer les Outliers"):
            if traitement_outlier != "Aucune":
                df = traiter_outliers(
                    df, numeric_cols, outliers_bool, traitement_outlier
                )
                st.success("Traitement des outliers effectué.")
                st.session_state["df_clean"] = df
                numeric_cols = df.select_dtypes(include=np.number).columns.tolist()
                outliers_bool_clean = detection_outliers(
                    df, numeric_cols, methode_outlier, key="zscore2"
                )
                st.write(outliers_bool_clean.sum())

        st.write("***")
        st.markdown(
            "### Vous pouvez maintenant passer à l'onglet suivant : ⚖️ Standardisation"
        )
...
with onglet5:
    if "df_clean" in st.session_state:
        ...
        st.write("***")
        st.markdown(
            "### Vous pouvez maintenant passer à l'onglet suivant : 🎯 Résumé & Exports"
        )
...
with onglet6:
    if "df_clean" in st.session_state:
        ...
        encoder_resume = st.session_state.get(
            "choix_encoder", "Aucun encodage (cible numérique)"
        )
        # Affichage
        st.markdown(
            f"""
        ...
        if st.session_state.get("standardized", False):
            ...
            with st.expander(
                "🔍 Détail des colonnes standardisées (moyenne et écart-type)"
            ):
        ...
        if st.button("📄 Générer le rapport PDF"):
            pdf_path, images = generer_rapport_pdf(
                df, df_clean, target_corr, selected_cols_corr, task, to_drop, corr
            )
            ...
            try:
                ...
                st.warning(
                    f"Erreur lors de la suppression des fichiers temporaires : {e}"
                )
